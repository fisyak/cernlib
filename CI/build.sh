...
mkdir -p /tmp
CHUARCH=64bit
if [ "x$TARGET" = "xtarball32bit" ]; then
CHUARCH=32bit
fi
if  [ "x$BUILDSYSTEM" = "ximake" ]; then
  export CERN=$(pwd)/tmp/cern
  if [ "x$BEAROPTION" = "xbear3" ]; then
    bear --output imakeGNU.json -- sh make_cernlib $CHUARCH $TOOLCHAIN $CERNLIB_XROOTD
    exit $?
  fi
  if [ "x$BEAROPTION" = "xbear" ]; then
    bear --cdb imakeGNU.json sh make_cernlib $CHUARCH $TOOLCHAIN $CERNLIB_XOOTD
    exit $?
  fi
...
#US This seems to override external settings
#US  if [ "$(uname)" = "Linux" ]; then
#US    extra_args=$extra_args"-DCERNLIB_USE_INTERNAL_XBAE=OFF -DCERNLIB_USE_INTERNAL_LAPACK=OFF"
#US  fi
     BEAR=""
  if [ "x$BEAROPTION" = "xbear" ]; then
  ...
  cd BUILD$TOOLCHAIN
  if [ "x$TARGET" != "xrpm" ] && [ "x$TARGET" != "xbrew" ]; then
    $CMAKE   ../  ${extra_args}  -DCERNLIB_NO_SUFFIX=1 -DCERNLIB_2022=yes -DCERNLIB_POSITION_INDEPENDENT_CODE=ON -DCMAKE_C_COMPILER=${CC} -DCERNLIB_BUILD_SHARED=OFF -DCMAKE_Fortran_COMPILER=${FC} -DCERNLIB_ENABLE_TEST=ON -DCMAKE_INSTALL_PREFIX=/2025.02 -DCERNLIB_XROOTD=${CERNLIB_XROOTD}
    $BEAR make -j 20 || exit $?
...
if [ "x$TARGET" = "xbrew" ] &&  [ "$(uname)" = "Darwin" ]; then
  cd $TOP/TEMP
...
if [ "x$TARGET" = "xrpm" ] &&  [ "$(uname)" = "Linux" ]; then
  cd $TOP/TEMP
