...
char* ovl_strcpy(char *a, const char *b) {
     ...
     char **argv;
{
  ...
  if( argc > iarg ) {
    ovl_strcpy( ifile, argv[iarg] );
    iarg++;
    if( argc > iarg )
      ovl_strcpy( ofile, argv[iarg] );
    else {
      /* use input file name and chop off extension */
      ovl_strcpy( ofile, ifile );
        ...
        if( par->range_count > 0 ) {
          if( par->type == KmTYPE_INT || par->type == KmTYPE_REAL ) {
            ...
            if( par->range_value[0][0] == '\0' )
              ...
              ovl_strcpy( line, par->range_value[0] );
            ...
            if( par->type == KmTYPE_OPTION ) {
              for( n = 0; n < par->range_count; n++ ) {
                if( par->option_text[n] != NULL ) {
                  ...
                  line[0] = '-';
                  ovl_strcpy( &line[1], par->range_value[n] );
                  output_kupval( cmd->name, par->name,
            ...
            const char *text )
{
  ...
  ovl_strcpy( &buf->buffer[buf->used], text );
  buf->used += len;
}
                ...
                Buffer *buf2 )
{
  ...
  ovl_strcpy( &buf1->buffer[buf1->used], buf2->buffer );
  buf1->used += buf2->used;
  buf2->used = 0;
  buf2->allocated = 0;
  free( buf2->buffer );
