 ...
 *
 * $Log$
 * Revision 1.2  1998/09/02 15:23:57  couet
 * - Min and Max are now always (re)defined. Previously they were only
 * for Motif 1.2 (mods needed for LINUX).
 *
 * Revision 1.1.1.1  1996/03/01 11:39:36  mclareni
 * Paw
 *
 */
/*CMZ :  2.06/01 16/11/94  14.42.44  by  Fons Rademakers*/
/*-- Author :*/
/*
...
/*
#ifndef SABER
static char sccsid[] = "@(#)Matrix.c    3.21 11/25/92";
#endif
*/
...
static XtResource resources[] = {
    ...
    { XmNevenRowBackground, XmCColor, XmRPixel, sizeof(Pixel),
          offset(matrix.even_row_background), XmRCallProc,
          (XtPointer)CopyBackground},
    { XmNoddRowBackground, XmCColor, XmRPixel, sizeof(Pixel),
...
void CopyBackground(widget, offset, value)
Widget widget;
int offset;
XrmValue *value;
{
...
void CopyForeground(widget, offset, value)
Widget widget;
int offset;
XrmValue *value;
{
 ...
 */
static void CreateDrawGC(), GetGridLineGC(),
...
static Boolean CvtStringToGridType();
...
XbaeMatrixWidget mw;
{
    ...
    if (!XmFontListInitFontContext(&context, mw->matrix.font_list))
        XtAppErrorMsg(XtWidgetToApplicationContext((Widget)mw),
                      ...
                      "XbaeMatrix: XmFontListInitFontContext failed, bad \
fontList",
                      NULL, 0);
    ...
    if (!XmFontListGetNextFont(context, &charset, &font))
        XtAppErrorMsg(XtWidgetToApplicationContext((Widget)mw),
                      "newFont", "badFont", "XbaeMatrix",
                      "XbaeMatrix: XmFontListGetNextFont failed, cannot get \
font from \
fontList",
                      NULL, 0);
  ...
  Dimension mgh, mgw, hlt;
    ...
    if (new->matrix.fixed_rows && new->matrix.fixed_columns) {
        XtAppWarningMsg(XtWidgetToApplicationContext((Widget)new),
                      ...
                      "XbaeMatrix: Cannot specify both fixedRows and \
fixedColumns",
                      (String *) NULL, (Cardinal *) NULL);
     ...
     */
  hlt = new->matrix.cell_highlight_thickness;
  if (new->matrix.cell_margin_width < hlt ||
      new->matrix.cell_margin_height < hlt ) {
    if (new->matrix.cell_margin_width < new->matrix.cell_margin_height) {
      mgw = 0;
      hlt = new->matrix.cell_margin_width;
      mgh = new->matrix.cell_margin_height - hlt;
    }
    else {
      mgh = 0;
      hlt = new->matrix.cell_margin_height;
      mgw = new->matrix.cell_margin_width - hlt;
    }
  } else {
    mgw = new->matrix.cell_margin_width - hlt;
    mgh = new->matrix.cell_margin_height - hlt;
...
Widget w;
{
    if (((CompositeWidget)XtParent(w))->composite.num_children > 3) {
        ...
        XtAppWarningMsg(XtWidgetToApplicationContext(w),
                        ...
                        "XbaeMatrix: Applications cannot add children to %s \
widgets",
                        params, &num_params);
        return;
    }
...
int row, column;
{
    ...
    /*
     * If this cell is selected, fill the cell with the selected background
     * color
     */
    if (mw->matrix.selected_cells[row][column])
        color = mw->matrix.selected_background;
    else if (mw->matrix.cell_background &&
             mw->matrix.cell_background[row][column] !=
             XbaeMatrixDefaultBackground)
        color = mw->matrix.cell_background[row][column];
    else {
        if (row & 1)
            color = mw->matrix.odd_row_background;
        else
            color = mw->matrix.even_row_background;
    }
    XSetForeground(XtDisplay(mw), gc, color);
...
*/
     ...
     */
    if (mw->matrix.selected_cells[row][column])
        color = mw->matrix.selected_foreground;
    else
...
Rectangle *expose;
{
    ...
    if (mw->manager.shadow_thickness)
        XmeDrawShadows(XtDisplay(mw), XtWindow(mw),
		       ...
		       2 * mw->manager.shadow_thickness, XmSINGLE_LINE);
}
...
#define EQ(field)       (current->field == new->field)
    ...
    if (NE(matrix.rows) &&
        ...
        XtAppWarningMsg(XtWidgetToApplicationContext((Widget)new),
                        "setValues", "rows",
                        "XbaeMatrix",
                        "XbaeMatrix: Number of rows changed but dependent \
resources did not",
                        NULL, 0);
    ...
    if (NE(matrix.columns) &&
        ...
        XtAppWarningMsg(XtWidgetToApplicationContext((Widget)new),
                        ...
                        "XbaeMatrix: Number of columns changed but dependent \
resources did not",
                        NULL, 0);
...
fixedColumns)",
    ...
    else if(new->matrix.cell_background && (NE(matrix.rows) || NE(
matrix.columns)))
            ...
            for (j = 0; j < mw->matrix.columns; j++) {
                if (!mw->matrix.cells[i][j]) {
                    XtAppWarningMsg(XtWidgetToApplicationContext((Widget)mw),
                                    ...
                                    "XbaeMatrix: NULL entry found in cell \
table",
                                    NULL, 0);
                    copy[i][j] = XtNewString("");
                }
    ...
    for (i = 0; i < mw->matrix.columns; i++)
        if (!mw->matrix.column_labels[i]) {
            XtAppWarningMsg(XtWidgetToApplicationContext((Widget)mw),
                            ...
                            "XbaeMatrix: NULL entry found in columnLabels \
array",
                            NULL, 0);
            copy[i] = XtNewString("");
    ...
    for (i = 0; i < mw->matrix.columns; i++) {
        if (!bad &&
            ...
            XtAppWarningMsg(XtWidgetToApplicationContext((Widget)mw),
                            ...
                            "XbaeMatrix",
                            "XbaeMatrix: Column label alignments array is too \
short",
                            NULL, 0);
            copy[i] = XmALIGNMENT_BEGINNING;
        }
...
XbaeMatrixWidget mw;
{
     ...
     * Malloc an array of row pointers
     */
    mw->matrix.cell_background = (Pixel **) XtMalloc(mw->matrix.rows * sizeof(
Pixel *));
...
XbaeMatrixWidget new;
{
    ...
    if (new->matrix.rows > current->matrix.rows) {
        ...
        for (i = current->matrix.rows; i < new->matrix.rows; i++) {
            ...
            for (j = 0; j < new->matrix.columns; j++)
                new->matrix.cell_background[i][j]=XbaeMatrixDefaultBackground;
        }
    ...
    if (mw->matrix.row_labels)
        mw->matrix.row_labels =
            (String *) XtRealloc((char *)mw->matrix.row_labels,
                                 ...
                                 sizeof(String));
    if (mw->matrix.cell_background)
        mw->matrix.cell_background = (Pixel **) XtRealloc((char *)
...
String value;
{
    if (row >= mw->matrix.rows || row < 0 ||
        ...
        XtAppWarningMsg(XtWidgetToApplicationContext((Widget)mw),
                        ...
                        "XbaeMatrix: Row or column parameter out of bounds for \
SetCell.",
                        NULL, 0);
        return;
    }
...
int row;
{
    ...
    if (row >= mw->matrix.rows || row < 0) {
        XtAppWarningMsg(XtWidgetToApplicationContext((Widget)mw),
                        ...
                        "XbaeMatrix: Row parameter out of bounds for \
DeselectRow.",
                        NULL, 0);
        return;
    }
...
int column;
{
    ...
    if (column >= mw->matrix.columns || column < 0) {
        XtAppWarningMsg(XtWidgetToApplicationContext((Widget)mw),
                        ...
                        "XbaeMatrix: Column parameter out of bounds for \
DeselectColumn.",
                        NULL, 0);
        return;
    }
...
int num_rows;
{
    ...
    if (num_rows >= mw->matrix.rows - mw->matrix.fixed_rows) {
        XtAppWarningMsg(XtWidgetToApplicationContext((Widget)mw),
                        ...
                        "XbaeMatrix: Attempting to delete too many rows in \
DeleteRows.",
                        NULL, 0);
        return;
    }
...
int num_columns;
{
    ...
    if (num_columns >= mw->matrix.columns - mw->matrix.fixed_columns) {
        XtAppWarningMsg(XtWidgetToApplicationContext((Widget)mw),
                        ...
                        "XbaeMatrix: Attempting to delete too many columns in \
DeleteColumns.",
                        NULL, 0);
        return;
    }
...
int num_colors;
{
    ...
    if (position < 0 || position + num_colors > mw->matrix.rows) {
        XtAppWarningMsg(XtWidgetToApplicationContext((Widget)mw),
                        ...
                        "XbaeMatrix: Position out of bounds or too many colors \
in SetRowColors.",
                        NULL, 0);
        return;
    }
     ...
     */
    if (mw->matrix.cell_background == NULL) {
        CreateColors(mw);
        for (i = 0; i < position; i++)
            for (j = 0; j < mw->matrix.columns; j++)
                mw->matrix.cell_background[i][j] = XbaeMatrixDefaultBackground;
...
Cardinal *nparams;
{
    ...
    if (*nparams != 1) {
        XtAppWarningMsg(XtWidgetToApplicationContext(w),
                        ...
                        "XbaeMatrix: Wrong number of parameters passed to \
EditCell action, needs 1",
                        NULL, 0);
        return;
    }
...
1",
    ...
    if (!strcmp(params[0], "True"))
        ...
        XtAppWarningMsg(XtWidgetToApplicationContext(w),
                        ...
                        "XbaeMatrix: Invalid parameter passed to CancelEdit \
action, must be True or \
False",
                        NULL, 0);
        return;
    }
...
1",
    ...
    if (!strcmp(params[0], "True"))
        ...
        XtAppWarningMsg(XtWidgetToApplicationContext(w),
                        ...
                        "XbaeMatrix: Invalid parameter passed to CommitEdit \
action, must be True or \
False",
                        NULL, 0);
        return;
...
XtPointer *data;
{
    ...
    if (*num_args != 0)
        XtAppWarningMsg(XtDisplayToApplicationContext(dpy),
                        ...
                        "XbaeMatrix",
                        "String to StringArray conversion needs no extra \
arguments",
                        NULL, NULL);
...
XtPointer *data;
{
    ...
    if (*num_args != 0)
        XtAppWarningMsg(XtDisplayToApplicationContext(dpy),
                        ...
                        "XbaeMatrix",
                        "String to WidthArray conversion needs no extra \
arguments",
                        NULL, NULL);
...
XtPointer *data;
{
    ...
    if (*num_args != 0)
        XtAppWarningMsg(XtDisplayToApplicationContext(dpy),
                        ...
                        "XbaeMatrix",
                        "String to MaxLengthArray conversion needs no extra \
arguments",
                        NULL, NULL);
...
XtPointer *data;
{
    ...
    if (*num_args != 0)
        XtAppWarningMsg(XtDisplayToApplicationContext(dpy),
                        ...
                        "XbaeMatrix",
                        "String to AlignmentArray conversion needs no extra \
arguments",
                        NULL, NULL);
...
XtPointer *data;
{
    ...
    if (*num_args != 0)
        XtAppWarningMsg(XtDisplayToApplicationContext(dpy),
                        ...
                        "XbaeMatrix",
                        "String to GridType conversion needs no extra \
arguments",
                        NULL, NULL);
    ...
    if (!strcmp(from->addr, "XmGRID_NONE"))
        grid_type = XmGRID_NONE;
    else if (!strcmp(from->addr, "XmGRID_LINE"))
        grid_type = XmGRID_LINE;
    else if (!strcmp(from->addr, "XmGRID_SHADOW_IN"))
        grid_type = XmGRID_SHADOW_IN;
    else if (!strcmp(from->addr, "XmGRID_SHADOW_OUT"))
