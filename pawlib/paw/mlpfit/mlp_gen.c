...
#include <cfortran/lapack.h>

#define EXTERNMLP_GEN
#ifndef EXTERN
#define EXTERN
#endif
#include "mlp_gen.h"
...
/* The following is needed to use the dgels routine from the LAPACK
   library in Reslin() 							    */
...
/*	for(in=0;in<NET.Nneur[0]; in++) printf("%le %le\n",
...
/* extern "C"Dllexport */void BFGSdir(int Nweights)
{
	...
	g = (dbl*) malloc(Nweights*sizeof(dbl));
...
/*	err1 = MLP_Test(0,0); 
	...
	if(NET.Debug>=4) printf("err depart= %lf\n",err1);   
...
/*	alpha2 = 0.05;
	...
	err2 = MLP_Test(0,0);
	(*Ntest) ++;
	if(NET.Debug>=4) printf("alpha, err= %le %le\n",alpha2,err2);   
...
/* compute error(w0) */
	err1 = MLP_Test(0,1);
	(*Ntest) ++; 
	if(NET.Debug>=4) printf("LinesearchHyb err depart= %lf\n",err1);   
...
/*           J.Schwindling 17-JUL-01                       */
/*                         remove ierr from dgels          */ 
/***********************************************************/   
...
#define LWORK 100000
	...
	integer Lwork = LWORK;
/*	integer rank; */
	/*AVunused doublereal rcond = -1;*/	/* use machine precision */	
...
/*		MLP_Out(rrin,rrout);*/
	...
	if(NET.Debug>=4) 
		{
		err = MLP_Test(0,0);
		printf("entry ResLin, err=MLP_Test(0,0), err= %lf\n",err); 
		}		 
/*                                                                */
...
/*                                                                */
	...
	DGELS("N",M,Nl,nrhs,HR,M,dpat,M,Work,Lwork,iret);
...
/*	ierr = dgelss_(&M,&Nl,&nrhs,HR,&M,dpat,&M,SV,&rcond,&rank,Work,&Lwork,
	...
	if(NET.Debug>=4) 
		{
		err = MLP_Test(0,0);
		printf("ResLin, apres tlsfor, err= %lf\n",err); 
		}		 
	free(dpat);
	free(wlin);
	free(HR);
...
#if(defined(WIN32))
#define MM 32767
return mini+(maxi-mini)*(dbl) rand()/(dbl) MM;
#else
#define MM 2147483647
return mini+(maxi-mini)*random()/MM;
#endif
}
...
double p;
int line,i;
int l,ll,ipat,nmax/*AV: unused,il,in,tf*/;
...
/* obsolete datacard 	 NLAY */			
		...
		if(*(s+1)=='S') 	/* FSPO */
			{
			sscanf(s,"%s %le",cc,&p);
			printf("Flat spot elimination parameter %lf\n",p);
			LEARN.delta = (dbl) p;
			}
		else if(*(s+1)=='I')	/* FILE */
			{
...
/*	else if(*s=='T')                 TFUN 
		...
		if(*(s+1)=='C')		/* DCAY */
			...
			sscanf(s,"%s %le",cc,&p);
			LEARN.Decay = p;
			printf("Learning parameter decay %lf\n",
						(double) LEARN.Decay);
			}
		if(*(s+1)=='B')		/* DBIN */	
			{	
...
int MLP_PrCFun(char *filename)
{
	...
	for(il=1; il<=NET.Nlayer-1; il++)
		...
		for(in=0; in<NET.Nneur[il]; in++)
			{
			fprintf(W,"      out%d[%d] = %le\n",il+1,in,
					(double) NET.Weights[il][in][0]);
			for(jn=1;jn<=NET.Nneur[il-1]; jn++)
				fprintf(W,"      +(%le) * out%d[%d]\n",
...
/*	PAT.Pond[ifile] = (type_pat *) realloc(PAT.Pond[ifile],
		ntot*sizeof(type_pat)); */
	for(j=PAT.Npat[ifile];j<ntot;j++)
		{
		PAT.Pond[ifile][j] = 1;
		}
...
/* examples */
	...
	PAT.Rin[ifile] = (type_pat**) malloc(ntot*sizeof(type_pat*));
...
/*	PAT.vRin[ifile] = (type_pat *) realloc(PAT.vRin[ifile], 
		ntot*(nin+1)*sizeof(type_pat)); */
	for(j=0; j<ntot; j++)
		{
		PAT.Rin[ifile][j] = &(PAT.vRin[ifile][j*(nin+1)+1]);
		PAT.vRin[ifile][j*(nin+1)] = 1; 
...
/*	tmp2 = (type_pat **) malloc(ntot*sizeof(type_pat*));
	...
	PAT.Rans[ifile] = (type_pat **) realloc(PAT.Rans[ifile], 
						ntot*sizeof(type_pat*)); 
